...
def get_trains():
    ...
    try:
        ...
        print(
            f"📥 Step 2: User input -> source='{source_input}', destination='{destination_input}', date='{journey_date_str}', class='{selected_class}'")
        ...
        print(
            f"📅 Step 3: Parsed date -> {journey_date} (in {days_before_journey} days), weekday={weekday}, month={month}")

        # --- Step 4: Build fast station lookup (deduplicated) ---
        print("🔁 Step 4: Building station lookup from dataset (fast mapping)")
        src_df = df[['source_code', 'source_name']].drop_duplicates().rename(
            columns={'source_code': 'code', 'source_name': 'name'})
        dst_df = df[['destination_code', 'destination_name']].drop_duplicates().rename(
            columns={'destination_code': 'code', 'destination_name': 'name'})
        ...
        # Fallback: if class filtered everything out but route exists, show route-only
        if results.empty and mapped_class:
            ...
            if not route_only.empty:
                ...
                results = route_only
                # IMPORTANT: in route-only fallback, we will show only one row per train regardless of class
                mapped_class = ""  # mark as no-class mode for dedup keys below
            else:
        ...
        # --- Strict de-dup so one train appears once per route ---
        before_dedup = len(results)

        # Decide identifier keys
        if 'train_no' in results.columns and results['train_no'].notna().any():
            base_keys = ['train_no', 'source_code', 'destination_code']
        else:
            base_keys = ['train_name', 'source_code', 'destination_code']

        # If a class was actually applied (not route-only), allow one per train per class.
        # If not, force single row per train across all classes.
        if mapped_class:
            dedup_keys = base_keys + ['class']
        else:
            dedup_keys = base_keys

        # Choose the “best” row per group before dropping duplicates:
        # lower avg_waitlist preferred, then higher punctuality_rate, then higher confirmation_chance
        for c in ['avg_waitlist', 'punctuality_rate', 'confirmation_chance']:
            if c not in results.columns:
                results[c] = None

        results = (
            results.sort_values(
                by=['avg_waitlist', 'punctuality_rate', 'confirmation_chance'],
        ...
        print(f"🧹 Dedup {dedup_keys}: {before_dedup} -> {len(results)} rows")
        ...
        for _, row in results.iterrows():
            ...
            train_list.append({
                ...
                "class": row.get("class"),
                "confirmation_chance": round(conf_val, 2),
                "confirmation_chance_text": f"{round(conf_val, 2)}%",
                ...
                "days_before_journey": int(row.get("days_before_journey")) if pd.notna(
                    row.get("days_before_journey")) else None,
